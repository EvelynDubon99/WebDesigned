  ...
  <body class="text-center">
   ...
   <section id="n1">
      ...
      if(isset($_REQUEST['usuario'])){
        $buscar = strtolower($_REQUEST['usuario']);
        if(empty($buscar)){
      ...
      ?>
          <div class="col-md-12">
                <form class="form-inline my-2 my-lg-0" action="buscar.php" metho="POST"  onSubmit="return validarForm(this)">
                <input class="form-control mr-sm-2"  name="usuario" type="search" placeholder="Usuario" aria-label="Search"
                valu="<?php echo $busqueda;?>">
          ...
          <div class=" table-responsive-md">
            ...
            <tbody>
              ...
              if($buscar == 'administrador'){
                  $rol= "OR ID_role LIKE '%1%' ";
              }else if($buscar == 'empleado'){
                $rol= "OR ID_role LIKE '%2%' ";
              }else if($buscar == 'logeado'){
                $rol= "OR ID_role LIKE '%3%' ";
              }
              
                $result=$mysqli->query("SELECT *
                FROM `usuario` WHERE (ID_user LIKE '%$buscar%' or usuario LIKE '%$buscar%' or nombre LIKE '%$buscar%' $rol )");

                $result2 =$mysqli->query("SELECT u.ID_user, u.nombre, u.usuario, u.apellido,
                  u.correo, u.telefono, u.direccion, r.rol  From usuario u inner join roles r on u.ID_role = r.ID_role
                  where (u.ID_user LIKE '%$buscar%' or u.usuario LIKE '%$buscar%' or u.nombre LIKE '%$buscar%' or r.rol LIKE '%$buscar%' )");
